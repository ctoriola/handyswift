...
export function ProviderSignUp() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { id, type, checked, value } = e.target as any;
    setFormData(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Redirect to provider dashboard after 1.5 seconds
        setTimeout(() => {
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8 md:p-10">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                    ...
                    <Button
                      type="submit"
                      disabled={loading}
                      className="w-full h-12 bg-[#007AFF] hover:bg-[#007AFF]/90 active:bg-[#D3D3D3] text-white rounded-xl shadow-md hover:shadow-lg transition-all mt-6 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
